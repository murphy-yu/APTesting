...
//  JNDTestingApplePay
//
//  Created by Murphy Yu on 21/07/2018.
...
@property (strong, nonatomic) PKPaymentAuthorizationViewController *paymentAuthorizationViewController;
...
- (IBAction)buyNow:(id)sender {
    if (![PKPaymentAuthorizationViewController class]) {
        ...
        NSAssert(NO, @"%s [Line %d] ", __PRETTY_FUNCTION__, __LINE__);
        return;
    }
    //检查当前设备是否可以支付
    ...
    if (@available(iOS 9.2, *)) {
        //ChinaUnionPay, good to go
    }
    else {
        NSLog(@"操作系统不支持ChinaUnionPay，请升级至9.2以上版本，且iPhone6以上设备才支持");
        NSAssert(NO, @"%s [Line %d] ", __PRETTY_FUNCTION__, __LINE__);
        return;
    }
    ...
    payRequest.merchantIdentifier = @"merchant.com.jndfax.JNDTestingApplePay";  //申请的merchantID
    ...
    //ApplePay控件
    if (!self.paymentAuthorizationViewController) {
        PKPaymentAuthorizationViewController *view = [[PKPaymentAuthorizationViewController alloc] initWithPaymentRequest:payRequest];
        view.delegate = self;
        self.paymentAuthorizationViewController = view;
    }
    
    
    [self presentViewController:self.paymentAuthorizationViewController animated:YES completion:nil];
...
- (void)paymentAuthorizationViewControllerDidFinish:(PKPaymentAuthorizationViewController *)controller
{
    [controller dismissViewControllerAnimated:YES completion:^{
        if(controller == self.paymentAuthorizationViewController) {
